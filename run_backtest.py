...
def when_to_grid(daily, execution, hourly, base: Config) -> str:
    """Generate results/WHEN-TO-GRID.md -- a decision guide, not a sales page.

    The question this answers is the one a reader actually has: given a market,
    should I run a grid, hold spot, or do neither? Every cell is measured.
    """
    from grid.data import iso_to_ms
    ...
    out = [
        ...
        "",
        "Generated by `python run_backtest.py --all`. Every figure is measured on real CoinW "
        "BTC_USDT data, not asserted.",
        "",
        "A grid strategy is usually sold as if it were free money. It is not: it is a specific "
        "trade -- you give up trend participation in exchange for harvesting oscillation, and you "
        "pay the exchange on every leg. This document is the honest map of when that trade is good.",
        "",
        "---",
        "",
        "## 1. Grid versus simply holding, in both numeraires",
        "",
        "The second half of this table is the one most comparisons omit. If you are a BTC holder, "
        "your question is not \"did I make USDT\" but **\"did I end up with more BTC\"** -- which is "
        "exactly what a coin-margined (幣本位) grid is meant to deliver.",
        "",
        "| Market | BTC moved | Grid (USDT) | Hold (USDT) | Winner | Grid ends with | Hold ends with | Winner |",
        "|---|---:|---:|---:|:--|---:|---:|:--|",
    ]

    for name, start, end, _note in windows:
        sub = execution.slice_ms(iso_to_ms(start), iso_to_ms(end))
        if len(sub) < 100:
            continue
        m = compute_metrics(run_backtest(sub, daily, base, label=name, hourly=hourly), sub)
        p0, p1 = float(sub.open[0]), float(sub.close[-1])
        btc_hold = base.initial_equity / (p0 * (1 + base.maker_fee))
        btc_grid = m.final_equity / p1
        usdt_win = "**grid**" if m.total_return > m.bh_return else "hold"
        coin_win = "**grid**" if btc_grid > btc_hold else "hold"
        out.append(
            f"| `{name}` | {pct(p1 / p0 - 1, 1)} | {pct(m.total_return)} | {pct(m.bh_return)} | "
            f"{usdt_win} | {btc_grid:.4f} BTC | {btc_hold:.4f} BTC | {coin_win} |"
        )

    out += [
        "",
        "**The pattern is unambiguous and it holds in both numeraires.** The grid wins when the "
        "market falls or chops sideways, and loses when it trends up. In the 2022 bear it finished "
        "with substantially *more BTC* than simply holding -- which is the coin-margined pitch, and "
        "it is real. In the bull windows it finished with a fraction of the BTC, which is the same "
    ...
    out += [
        ...
        "",
        "The most common retail intuition is that a denser grid catches more moves and therefore "
        "earns more. Measured on the two-year `flat-2024-26` window, holding the covered range "
        "roughly constant and varying only the number of levels:",
        "",
        "| Levels | Spacing | Round trips | Fee drag | Return | Max DD |",
        "|---:|---:|---:|---:|---:|---:|",
    ]

    sub = execution.slice_ms(iso_to_ms("2024-08-01"), iso_to_ms("2026-08-01"))
    density = [("6", 1.00, 6), ("12", 0.50, 12), ("25", 0.24, 25), ("50", 0.12, 50), ("100", 0.06, 100)]
    for tag, k, n_lv in density:
        cfg = base.with_(k_spacing=k, n_levels=n_lv, s_cap=0.30)
        res = run_backtest(sub, daily, cfg, label=tag, hourly=hourly)
        m = compute_metrics(res, sub)
        spacing = float(np.median([s.spacing for s in res.signals])) if res.signals else float("nan")
        star = " **(default)**" if n_lv == base.n_levels else ""
        out.append(
            f"| {tag}{star} | {upct(spacing)} | {m.round_trips} | **{upct(m.fee_drag,1)}** | "
            f"{pct(m.total_return)} | {upct(m.max_dd)} |"
        )

    out += [
        "",
        "Round trips rise with density exactly as the intuition predicts. So does fee drag -- from "
        "single digits to well over half of gross spread capture. **At 100 levels the exchange "
        "takes more of the gross profit than the trader does.** Drawdown falls, because each level "
        "is smaller, so a dense grid is a lower-risk way to make no money.",
        "",
        "### Ruling out our own fill rule as the cause",
        "",
        "This engine normally allows one fill per level per UTC day (rule F7), which is stricter "
        "than a real bot and could by itself cap a dense ladder. A conclusion that dense grids are "
        "bad has to survive removing that cap, so here it is removed -- levels re-arm on the next "
        "bar:",
        "",
    ...
    for tag, k, n_lv in density:
        ...
        for refill in (False, True):
            cfg = base.with_(k_spacing=k, n_levels=n_lv, s_cap=0.30, refill_intraday=refill)
            m = compute_metrics(run_backtest(sub, daily, cfg, label=tag, hourly=hourly), sub)
            row.append((m.round_trips, m.total_return))
        out.append(
            f"| {tag} | {row[0][0]} | {pct(row[0][1])} | {row[1][0]} | {pct(row[1][1])} |"
        )

    out += [
        "",
        "Relaxing the cap gives dense grids *more* fills and makes them **worse**, because the "
        "extra fills do not cover their own fees. The finding is a property of the cost structure, "
        "not of our fill model.",
        "",
        "---",
        "",
        "## 3. What we deliberately did NOT test, and why",
        "",
        "Several grid variants sold in the market are missing from this study. That is a choice, "
        "not an oversight, and the reason matters more than a number would.",
        "",
        "| Variant | Status | Why |",
        "|---|---|---|",
        ...
        "",
        "**What it would take to add them honestly:** the historical funding-rate series for the "
        "instrument, the margin-tier table, and the mark-price methodology -- then the same fill "
        "model, plus a liquidation check on every bar. That is a second study, not a parameter.",
        "",
        "**The intuition worth stating anyway,** clearly labelled as reasoning rather than "
        "measurement: in a fast directional move, a leveraged directional position captures the "
        "move while a grid is busy selling into it. Everything measured above is consistent with "
        "that. If you are confident about direction, a grid is the wrong instrument -- it is "
        "designed for the case where you are *not*.",
        "",
        "---",
        "",
        "## 4. The one-paragraph answer",
        "",
        "Run a grid when you expect a range or a decline, when you want to be paid for volatility "
        "without taking a directional view, and when your fee tier leaves room above "
        f"`2f/(1-f)` = {upct(base.breakeven, 4)} per round trip. Do not run one because it looks "
        "like free money, do not add levels hoping for more profit, and do not run one at all if "
        "you believe the market is going up -- in that case buy spot and hold, which beat this "
        "strategy in every trending window tested, in both USDT and BTC terms.",
        "",
    ]
    return "\n".join(out)
...
def main() -> int:
    ...
    (RESULTS_DIR / "WHEN-TO-GRID.md").write_text(
        when_to_grid(daily, execution, hourly, cfg), encoding="utf-8"
    )
    print("  results/WHEN-TO-GRID.md")
