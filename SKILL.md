...
This is not theoretical. Sweeping `K` across six independent windows, return improves monotonically
in every one as spacing widens, and fee drag falls from 19.8% to 4.8%. The mechanism is the whole
argument: tighter grids fill more often and hand the difference to the exchange.
...
|---|---:|---|---|
| `k_spacing` | 1.00 | Spacing = K × daily ATR% | Return is monotone in K across all six tested windows; below 0.5, fee drag dominates |
...
| `cap_range` / `cap_uptrend` / `cap_downtrend` | 0.50 / 0.60 / 0.15 | Inventory cap by regime | **The dominant risk lever.** Full-run max drawdown runs 26.5% / 28.7% / 39.9% at caps of 0.20 / 0.30 / 0.50 |
...
| `adx_threshold` | 25 | Trend-strength gate | Wilder's conventional value; not tuned. Measured median ADX14 is 26.2 |
...
- **This strategy is structurally long BTC. It is not market-neutral, not hedged, and not
  ...
  of **35.4%**, and over the full 2019-2026 run its maximum drawdown was **39.9%**.
...
- **It does not beat holding BTC.** Over the full 2019-2026 run it returned **+52.7%** against
  **+1,598.9%** for buy-and-hold. It won 3 of 7 windows against buy-and-hold and 3 of 7 against an
  ...
  for roughly half the drawdown (39.9% against 77.2% over the full run).
...
- Moving averages and ADX are lagging and generate false signals in sideways markets. The regime
  gate has **no directional predictive power** — measured forward 5-day return in DOWNTREND is
  −0.05%, indistinguishable from zero. It is a risk control, not an alpha source.
